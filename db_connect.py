    ...
    def add_warning(self, user_id):
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute("SELECT * FROM user_warnings WHERE user_id = %s", (user_id,))
                result = cursor.fetchone()
                if result:
                    # Если запись уже есть, обновляем ее данные
    ...
    def check_warnings(self, user_id):
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute("SELECT warn_count FROM user_warnings WHERE user_id = %s", (user_id,))
                result = cursor.fetchone()
                if result:
                    warn_count = result['warn_count']
                    if warn_count >= 5:
                        # Если количество предупреждений достигло 5, выполняем бан пользователя
                        print(f"Бан пользователя с ID {user_id} из-за 5 предупреждений.")
                        # Ваш код для выполнения бана пользователя
                    elif warn_count >= 3:
                        # Если количество предупреждений достигло 3, выполняем мут пользователя на день
                        print(f"Мут пользователя с ID {user_id} на день из-за 3 предупреждений.")
