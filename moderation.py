...
class Moderation(commands.Cog):
    ...
    # kick - КИК
    @commands.slash_command(description="Кикает пользователя с сервера.")
    @commands.has_permissions(kick_members=True)
    async def kick(
        self, inter, member: disnake.Member, reason: str = "Нарушение правил!"
    ):
    ...
    # /ban - БАН
    @commands.slash_command(description="Банит пользователя на сервере.")
    async def ban(
        self, interaction, member: disnake.Member, *, reason: str = "Нарушение правил!"
    ):
        try:
    ...
    # /clear - ОЧИСТКА ЧАТА
    @commands.slash_command(description="Удаляет указанное количество сообщений.")
    @commands.has_permissions(manage_messages=True)
    async def clear(self, inter, amount: int):
        deleted = await inter.channel.purge(limit=amount + 1)
        await inter.response.send_message(
            f"Успешно удалено {len(deleted) - 1} сообщений!", ephemeral=True
        )

    # /mute - МУТ
    @commands.slash_command(description="Мутит человека на сервере.")
    async def mute(self, interaction, member: disnake.Member, duration: int = None):
        ...
        try:
            ...
            await asyncio.sleep(
                1
            )  # Подождите немного, чтобы убедиться, что взаимодействие завершено
            await interaction.edit_original_message(
                content=f"{member.display_name} был заглушён на {duration} секунд."
            )
            ...
            if duration:
                ...
                try:
                    ...
                    await interaction.edit_original_message(
                        content=f"{member.display_name} теперь свободен от мута."
                    )
                except Exception as e:
    ...
    # /unmute - РАЗМУТ
    @commands.slash_command(description="Снимает мут с пользователя")
    @commands.has_permissions(manage_messages=True)
    async def unmute(
        ...
        if mute_role in member.roles:
            await member.remove_roles(mute_role)
            await inter.response.send_message(
                f"Мут с пользователя {member.display_name} снят."
            )
        else:
            await inter.response.send_message("У пользователя нет мута.")

    # /unban - РАЗБАН
    @commands.slash_command(description="Разбанивает человека на сервере.")
    async def unban(self, interaction, *, member_id: int):
        try:
            user = await self.bot.fetch_user(member_id)
            await interaction.guild.unban(user)
            await interaction.response.send_message(
                content=f"Пользователь {user.display_name} разбанен."
            )
        except disnake.NotFound:
            await interaction.response.send_message(
    ...
    async def nickname(
        ...
        if inter.author.guild_permissions.manage_nicknames:
            ...
            await inter.response.send_message(
                f"Никнейм пользователя {member.display_name} изменён на {new_nick}."
            )
        else:
            await inter.response.send_message(
    ...
    async def log_activity(self, user_id, activity_type):
        try:
            with self.db.conn.cursor() as cursor:
                ...
                cursor.execute(
                    "INSERT INTO user_activity (user_id, activity_date, activity_type) VALUES (%s, %s, %s)",
    ...
    # /activity - АКТИВНОСТЬ
    @commands.slash_command(description="Показывает активность пользователя на сервере")
    async def activity(
        self, inter: disnake.ApplicationCommandInteraction, member: disnake.Member
    ):
        try:
            with self.db.conn.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM user_activity WHERE user_id = %s", (member.id,)
                )
                activities = cursor.fetchall()
                if activities:
                    activity_list = "\n".join(
                        f"{activity['activity_date']}: {activity['activity_type']}"
            ...
            print(
                "- [Mafanya] Ошибка при запросе активности пользователя из базы данных:"
            )
            print(e)

    # /avatar - АВАТАР
    @commands.slash_command(description="Показывает аватар человека на сервере.")
    async def avatar(self, interaction, member: disnake.Member = None):
        user = (
            member or interaction.author
        )  # Обеспечиваем возможность выбора пользователя, или использование автора команды по умолчанию
        embed = disnake.Embed(
            title=f"Avatar of {user.display_name}", color=0x2F3136
        )  # Название эмбеда с именем пользователя
        embed.set_image(
            url=user.display_avatar.url
        )  # Установка изображения аватара пользователя
